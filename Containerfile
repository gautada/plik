...
RUN apk add --no-cache \
    ...
    curl uv
WORKDIR /opt
RUN git clone --branch ${GITHUB_TAG} https://github.com/gautada/plik.git uvicornapp
WORKDIR /opt/uvicornapp
RUN uv venv .venv \
...
COPY --from=BUILD /opt/uvicornapp /opt/uvicornapp
RUN chown ${USER}:${USER} -R /opt/uvicornapp /home/${USER}
WORKDIR /home/${USER}
